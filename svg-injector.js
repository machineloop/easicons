...
(function (window, document) {
  ...
  var injectElement = function (el, evalScripts, pngFallback, callback) {
    ...
    loadSvg(imgUrl, function (svg) {
      ...
      var imgData = [].filter.call(el.attributes, function (at) {
        return (/^data-\w[\w\.\w\:\w\-]*$/).test(at.name);
      });
      if (imgData.length < 1) {
      ...
      if (window._classifySVGs) {
        window._classifySVGs();
      }
      console.log(window._classifySVGs);
    });
  };
   ...
   */
  var SVGInjector = function (elements, options, done, callback) {
